...
final class Flatsome_Envato_Registration extends Flatsome_Base_Registration {
	...
	/**
	 * Exchange the purchase ID into a purchase code, then register.
	 *
	...
	public function register( $purchase_id ) {
		...
		if ( empty( $token ) ) {
			return new WP_Error( 'missing-token', __( 'Missing token.', 'flatsome' ) );
		}
	...
	public function unregister() {
		$this->delete_options();
		return new WP_Error( 'warning', __( 'No registration was found for your domain. The registration has only been removed locally.', 'flatsome' ) );
	}
