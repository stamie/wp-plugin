...
$panes = array();
	...
	$content = flatsome_contentfix(do_shortcode($content));
  ...
  if ( $visibility ) $wrapper_class[] = $visibility;
	...
	if( is_array( $GLOBALS['tabs'] )){
		...
		foreach( $GLOBALS['tabs'] as $key => $tab ){
			...
			$tabs[] = '<li class="tab'.$active.' has-icon"><a href="#rent-a-boat-'.$i.'" name="rent-a-boat-'.$i.'"><span><h2>'.$tab['title'].'</h2></span></a></li>';
