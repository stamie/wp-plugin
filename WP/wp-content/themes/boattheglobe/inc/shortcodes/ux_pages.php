    ...
    extract(shortcode_atts(array(
        ...
        $childpages = get_pages( array( 'include' => $ids, 'sort_column' => $orderby , 'sort_order' => $order ) );
          ...
          if($parent) {
            ...
            if(!is_numeric($parent2)){
              $id = get_page_by_path( $parent2, OBJECT, 'destination' );
              $parent2 = $id->ID;
            }
            $post_id = $parent2;
          }
          ...
          if ($parent)
            ...
            $childpages = get_destinations2( array( 'post_parent' => $post_id, 'orderby' => $orderby, 'order' => $order ) );


          if(!$childpages) echo '<p class="lead shortcode-error text-center">Sorry, no pages or destinations  was found</p>';
      }
      ...
      ?>
        <div class="<?php echo implode(' ', $classes_col); ?>" <?php echo $animate;?>>
          ...
          <a class="plain" href="<?php echo get_the_permalink($page->ID); ?>" title="<?php echo $page->_yoast_wpseo_focuskw; ?>" target="<?php echo $target; ?>">
            <div class="<?php echo implode(' ', $classes_box); ?>">
                  ...
                  <div class="<?php echo implode(' ', $classes_text); ?>" <?php echo get_shortcode_inline_css($css_args); ?>>
                        <div class="box-text-inner">
                            <p><?php echo $page->_yoast_wpseo_focuskw; ?></p>
                        </div><!-- box-text-inner -->
                  </div><!-- box-text -->
              </div><!-- .image-box .box -->
            </a>
