...
function shortcode_latest_from_blog($atts, $content = null, $tag) {
  ...
  if($visibility == 'hidden') return;
	...
	// If custom ids
	$pt = 'post';
	if ( !empty( $ids ) || !empty( $ids0 )|| !empty( $ids1 )|| !empty( $ids2 )) {
		...
		if(!empty( $ids0 )){
				$ids0 = explode( ',', $ids0);
				$ids = array_map( 'trim', $ids0 );
		$pt = strtolower('post');
			}
		if(!empty( $ids1 )){
				$ids1 = explode( ',', $ids1);
				$ids= array_map( 'trim', $ids1 );
		$pt = strtolower('destination');
			}
		if(!empty( $ids2 )){
				$ids2= explode( ',', $ids2);
				$ids = array_map( 'trim', $ids2 );
		$pt = strtolower('port');
			}
