...
<div id="wp-post-nav-instructions">
  ...
  <p>Version 1.0.0 of WP Post Nav introduces some developer options.  Details of the available hooks / options are included in the examples.txt file in the root of the plugin folder.</p>
  ...
  <table>
    ...
    <tr>
      ...
      <td>To allow users to modify the length of the excerpt on the navigation, we create our own excerpt from the content.  For some websites / themes where the pages are made up of shortcodes and custom content, this will not work correctly.  In order to use WP Post Nav, simple '__return_false' this function to use the Wordpress built in excerpt function.  To then adjust the excerpt length, you will need to use the Wordpress filter <a href="https://developer.wordpress.org/reference/hooks/excerpt_length/">'excerpt_length'</a> to adjust the excerpt length.</td> 
      
    </tr>
    <tr>
      <td class="wp-post-nav-centered"><strong>gettext (WordPress built in function)</strong></td>
      <td>Translate the plugin</td>
      <td>Use this function to make text modifications and translate the text on the front end.  Look at the examples.txt file but you can now translate the wording on the NAV.  The previous filter has been removed.</td> 
    </tr>
  </table>
</div>
