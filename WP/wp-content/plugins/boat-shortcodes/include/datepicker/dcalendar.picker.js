...
+function ($) {
 	/*var $inSwow = $('#mycalendar');
	var $inSwow2 = $('#mycalendar2');

	var $show = $inSwow;
	//$("#date_from_felso").click(function (e) {
		$show = $inSwow2;
	//});
	if ($(this).attr('id') == 'date_from_felso') {
		var $show = $inSwow2;
	} */														
	//var inshow  = '#mycalendar';
	var $show = null;
	...
	DCalendar.prototype = {
		...
		disabledDate: function (date) {
			var that = this, rangeFrom = null, rangeTo = null, rangeMin = null, rangeMax = null, min = null, max = null,
				now = new Date(Date.now()), //-(86400000 * diff)),
				 today = new Date(now.getFullYear(), now.getMonth(), now.getDate());

			if (that.minDate) min = that.minDate === "today" ? today : new Date(that.minDate-(diff*86400000));
			if (that.maxDate) max = that.maxDate === "today" ? today : new Date(that.maxDate-(diff*86400000));
		...
		getDays : function (newDate, callback) {
			...
			for(var i = 1; i <= 6; i++){
				...
				while(day <= days+diff) {
					d.setDate(day);
					var dayOfWeek = d.getDay()-diff;
					if (dayOfWeek < 0) {dayOfWeek = 6;}

					if (d.getTime()-(86400000 * diff) == today.getTime()) week[dayOfWeek].addClass('current');

                    if (that.disabledDate(d)) week[dayOfWeek].addClass('disabled');

					if(i === 1 && dayOfWeek === 0){ /*
						if (d.getTime()+(86400000 * diff) == that.selected.getTime()) week[dayOfWeek].addClass('selected');
						

						week[dayOfWeek].html('<a href="javascript:void(0);">' + 1 + '</a>');*/
						break;
					} else if (dayOfWeek < 6 ) {
					    ...
					    if (d.getTime() == that.selected.getTime() && (dayOfWeek)>=0) week[dayOfWeek].addClass('selected');
		...
		create : function(){
			var that = this,
				...
				calhead = $('<button type="button" class="close">Close</button><button type="button" class="cleare">Clear</button><section class="calendar-top-selector"><span class="calendar-prev">&lsaquo;</span><span class="calendar-curr-month"></span><span class="calendar-next">&rsaquo;</span></section>'),
				datesgrid = $('<section class="calendar-grid">'
							+ '<div class="calendar-labels"><span>Mo</span><span>Tu</span><span>We</span><span>Th</span><span>Fr</span><span>Sa</span><span>Su</span></div>'
...
*/
		...
		show : function () {
			$show.attr('datepicker-display', 'on');
		...
		hide : function (callback) {
			...
			that.calendar.parent().fadeOut(function () {
				$show.removeAttr('datepicker-display');
	...
	$.fn.dcalendarpicker = function(opts){
		return $(this).each(function(){
			...
			$(".close").on('click', function (e) {
				//if(e.keyCode != 27) return;
				if($show.attr('datepicker-display') && $show.attr('datepicker-display')=="on")
					that.data(DCAL_DATA).hide();
					
				
			});
			$(".cleare").on('click', function (e) {
				//if(e.keyCode != 27) return;
				if($show.attr('datepicker-display') && $show.attr('datepicker-display')=="on"){
					//that.data(DCAL_DATA).hide();
					$("#date_from").val('');
