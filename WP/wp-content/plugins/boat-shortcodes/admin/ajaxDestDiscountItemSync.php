...
// echo $queryDestinations;
   ...
   $rowsDestination = (is_array($rowsDestination) && count($rowsDestination) > 0)?$rowsDestination[0]:null;
    ...
    $return.= '<button class="all-discountItem down">Összes megjelenítése</button><button class="all-discountItem up hidden">Összes elrejtése</button><ol class="tree dest-discountItem">';
    ...
    $edit = '<button type="button" class="dest-discountItem button" attr-dest="'.$rowsDestination->ID.'" attr-prefix="'.$prefix_id.'" attr-xml="'.$xml_id.'"> <span class="dashicons dashicons-edit"> </span> </button>';
    //lefelé megy
    ...
    if ($bool){ // van gyereke
        $return .= $li.$button.$edit.$data.selectorsDiscountItem($rowsDestination2->ID, $xml_id, $parentId).'<ol>'; 
        ...
        if (++$index==1){ 
            $return .= $li.$edit.$data.selectorsDiscountItem($rowsDestination2->ID, $xml_id, $parentId);
            $return .= '</li>';
        } else{
            $return .= '</ol></li>';
        }
...
<div class="acordion-panel">
    <button class="help accordion help-dest-discountItem"> </button>
    <?php
    $return2 = selectNewDiscounts($xml_id);
        if ($return2 != ''):
?>      
    <button class="new accordion new-dest-discountItem"> </button>
    <?php else: ?>
    <button class="new accordion new-dest-discountItem" style="display: none;"> </button>
...
<?php   if ($return2 != ''):
    ...
    <div class="panel"><div class="helper-background discountItem">
    <?=$return2?>
    </div>
    </div>
    <?php endif;?>
...
var acc = document.getElementsByClassName("new-dest-discountItem");
    ...
    jQuery('.dest-discountItem.button').on('click', function (event) {
        ...
        var url = "/wp-content/plugins/boat-shortcodes/admin/ajaxSubmitDiscountItemForDest.php";
    ...
    //Összes Lenyitása
    jQuery("button.all-discountItem.down").on('click',function(){
        var childrenLi     = jQuery('ol.tree.dest-discountItem').find('li');
        var childrenButton = jQuery('button.all-discountItem.up');
    ...
    jQuery("button.all-discountItem.up").on('click',function(){
        var childrenLi     = jQuery('ol.tree.dest-discountItem').children('li');
        var childrenButton = jQuery('button.all-discountItem.down');
