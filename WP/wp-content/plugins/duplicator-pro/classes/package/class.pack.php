    ...
    public function create_scan_report()
    {
        ...
        try {
            ...
            $report['ARC']['Status']['PackageIsNotImportable'] = !($report['ARC']['Status']['CanbeMigratePackage'] && !$this->Archive->ExportOnlyDB
                ...
                && (!$report['ARC']['Status']['HasNotImportableSites'] || \Duplicator\Addons\ProBase\License\License::isBusiness()));
            ...
            if (!empty($json)) {
                DupProSnapLibIOU::fwrite($fp, $json);
            } else {
    ...
    public function build_integrity_check()
    {
        ...
        if (class_exists('ZipArchive') && ($this->build_progress->current_build_mode == DUP_PRO_Archive_Build_Mode::ZipArchive)) {
            ...
            if ($res !== true) {
                ...
                DUP_PRO_LOG::traceError($consistency_error);
            } else {
                $failed = false;
                foreach ($this->Installer->getInstallerPathsForIntegrityCheck() as $path) {
                    if ($zip->locateName($path) === false) {
                        $failed = true;
                        DUP_PRO_Log::traceError(DUP_PRO_U::__("Couldn't find $path in archive"));
                    }
                }
                ...
                if ($failed) {
                    ...
                    DUP_PRO_LOG::trace("Zip for package $this->ID didn't passed consistency test");
                } else {
