...
class DUP_PRO_UI_Notice
{
    ...
    public static function showQuickFixNotice()
    {
        ...
        $system_global = \DUP_PRO_System_Global_Entity::get_instance();
        $html          = '<b class="title"><i class="fa fa-exclamation-circle fa-3 color-alert" ></i> ' . DUP_PRO_U::__('Duplicator Pro Quick Fix') . '</b></br>';
        $html          .= '<b>' . DUP_PRO_U::__('Error(s) Detected:') . ' </b>';
        $html          .= DUP_PRO_U::__('Please perform the actions below then build the package again.') . "</br>";
        foreach ($system_global->recommended_fixes as $fix) {
            if ($fix->recommended_fix_type == DUP_PRO_Recommended_Fix_Type::Text) {
                $html .= "<i class='fa fa-question-circle color-alert' data-tooltip='" . esc_attr($fix->error_text) . "'></i>&nbsp; " . $fix->parameter1 . "</br>";
            } else if ($fix->recommended_fix_type == DUP_PRO_Recommended_Fix_Type::QuickFix) {
                $html .= '<span id ="quick-fix-' . $fix->id . '">'
