...
class DUP_PRO_MU
{
    ...
    public static function getSubsiteInfo($siteObj, $filteredTables = array(), $filteredPaths = array())
    {
        ...
        $site_info->upload_path    = DupProSnapLibIOU::getRelativePath($uploadPath, DUP_PRO_Archive::getTargetRootPath(), true);
        ...
        $site_info->filtered_paths = array_values(array_filter($filteredPaths, function ($path) use ($uploadPath, $subsiteID) {
                if (($relativeUpload = DupProSnapLibIOU::getRelativePath($path, $uploadPath)) === false
