...
class DUPX_Plugin_item
{
    ...
    {
        return in_array($this->data['slug'], DUPX_Params_Manager::getInstance()->getValue(DUPX_Params_Manager::PARAM_IGNORE_PLUGINS));
    }

    public function isForceDisabled()
    {
        return in_array($this->data['slug'], DUPX_Params_Manager::getInstance()->getValue(DUPX_Params_Manager::PARAM_FORCE_DIABLE_PLUGINS));
    }
    /**
    ...
    public function getPluginPath()
    {
        ...
        $mainDir      = false;
        if ($this->isMustUse()) {
            $mainDir = $paramManager->getValue(DUPX_Params_Manager::PARAM_PATH_MUPLUGINS_NEW);
        } else if ($this->isDropIns()) {
            $mainDir = $paramManager->getValue(DUPX_Params_Manager::PARAM_PATH_CONTENT_NEW);
        } else {
            $mainDir = $paramManager->getValue(DUPX_Params_Manager::PARAM_PATH_PLUGINS_NEW);
        }
    ...
    public function uninstall()
    {
        ...
        try {
            ...
            if (($pluginPath = $this->getPluginPath()) == false) {
                DUPX_Log::info('PLUGINS: can\'t remove plugin '.$this->slug.' because doesn\'t exist');
            } else {
                $origFileManager->addEntry($this->slug, $pluginPath, DUPX_Orig_File_Manager::MODE_MOVE);
            }
        }
