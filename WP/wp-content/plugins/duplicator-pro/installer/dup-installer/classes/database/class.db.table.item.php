...
class DUPX_DB_Table_item
{
    ...
    public function getNewName()
    {
        ...
        switch (DUPX_InstallerState::getInstType()) {
            ...
            case DUPX_InstallerState::INSTALL_MULTISITE_SUBFOLDER:
                return $paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_TABLE_PREFIX) . $this->getNameWithoutPrefix(true);
            case DUPX_InstallerState::INSTALL_STANDALONE:
                if ($this->subsiteId === $paramsManager->getValue(DUPX_Params_Manager::PARAM_SUBSITE_ID) &&
                    $this->subsiteId > 1) {
                    // convert standalon subsite prefix
                    return $paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_TABLE_PREFIX) . $this->getNameWithoutPrefix(false);
                } else {
                    return $paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_TABLE_PREFIX) . $this->getNameWithoutPrefix(true);
                }
    ...
    protected function standAloneExtractCheck()
    {
        ...
        $standaloneId = DUPX_Params_Manager::getInstance()->getValue(DUPX_Params_Manager::PARAM_SUBSITE_ID);
    ...
    protected function addSiteOnMultisiteCheck()
    {
        ...
        if (
            ...
            $this->subsiteId != DUPX_Params_Manager::getInstance()->getValue(DUPX_Params_Manager::PARAM_SUBSITE_ID)
        ) {
            return false;
        }
    ...
    public function extract()
    {
        ...
        $tablesVals = DUPX_Params_Manager::getInstance()->getValue(DUPX_Params_Manager::PARAM_DB_TABLES);
