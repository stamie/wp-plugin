...
class DUPX_ArchiveConfig
{
    ...
    {
        self::manageEmptyPathAndUrl(DUPX_Params_Manager::PARAM_PATH_WP_CORE_NEW, DUPX_Params_Manager::PARAM_SITE_URL);
        self::manageEmptyPathAndUrl(DUPX_Params_Manager::PARAM_PATH_CONTENT_NEW, DUPX_Params_Manager::PARAM_URL_CONTENT_NEW);
        self::manageEmptyPathAndUrl(DUPX_Params_Manager::PARAM_PATH_UPLOADS_NEW, DUPX_Params_Manager::PARAM_URL_UPLOADS_NEW);
        self::manageEmptyPathAndUrl(DUPX_Params_Manager::PARAM_PATH_PLUGINS_NEW, DUPX_Params_Manager::PARAM_URL_PLUGINS_NEW);
        self::manageEmptyPathAndUrl(DUPX_Params_Manager::PARAM_PATH_MUPLUGINS_NEW, DUPX_Params_Manager::PARAM_URL_MUPLUGINS_NEW);
    ...
    public static function getDefaultPathUrlValueFromParamKey($paramKey)
    {
        $paramsManager = DUPX_Params_Manager::getInstance();
        switch ($paramKey) {
            case DUPX_Params_Manager::PARAM_SITE_URL:
                return $paramsManager->getValue(DUPX_Params_Manager::PARAM_URL_NEW);
            case DUPX_Params_Manager::PARAM_URL_CONTENT_NEW:
                return $paramsManager->getValue(DUPX_Params_Manager::PARAM_URL_NEW) . '/wp-content';
            case DUPX_Params_Manager::PARAM_URL_UPLOADS_NEW:
                return $paramsManager->getValue(DUPX_Params_Manager::PARAM_URL_CONTENT_NEW) . '/uploads';
            case DUPX_Params_Manager::PARAM_URL_PLUGINS_NEW:
                return $paramsManager->getValue(DUPX_Params_Manager::PARAM_URL_CONTENT_NEW) . '/plugins';
            case DUPX_Params_Manager::PARAM_URL_MUPLUGINS_NEW:
                return $paramsManager->getValue(DUPX_Params_Manager::PARAM_URL_CONTENT_NEW) . '/mu-plugins';
            case DUPX_Params_Manager::PARAM_PATH_WP_CORE_NEW:
                return $paramsManager->getValue(DUPX_Params_Manager::PARAM_PATH_NEW);
            case DUPX_Params_Manager::PARAM_PATH_CONTENT_NEW:
                return $paramsManager->getValue(DUPX_Params_Manager::PARAM_PATH_NEW) . '/wp-content';
            case DUPX_Params_Manager::PARAM_PATH_UPLOADS_NEW:
                return $paramsManager->getValue(DUPX_Params_Manager::PARAM_PATH_CONTENT_NEW) . '/uploads';
            case DUPX_Params_Manager::PARAM_PATH_PLUGINS_NEW:
                return $paramsManager->getValue(DUPX_Params_Manager::PARAM_PATH_CONTENT_NEW) . '/plugins';
            case DUPX_Params_Manager::PARAM_PATH_MUPLUGINS_NEW:
                return $paramsManager->getValue(DUPX_Params_Manager::PARAM_PATH_CONTENT_NEW) . '/mu-plugins';
            default:
    ...
    {
        return $this->wp_tableprefix != DUPX_Params_Manager::getInstance()->getValue(DUPX_Params_Manager::PARAM_DB_TABLE_PREFIX);
    }

    public function getTableWithNewPrefix($table)
    {
        $search  = '/^' . preg_quote($this->wp_tableprefix, '/') . '(.*)/';
        $replace = DUPX_Params_Manager::getInstance()->getValue(DUPX_Params_Manager::PARAM_DB_TABLE_PREFIX) . '$1';
    ...
    {
        $paramsManager = DUPX_Params_Manager::getInstance();
        $mainUploadUrl = $paramsManager->getValue(DUPX_Params_Manager::PARAM_URL_UPLOADS_OLD);
        if ($subsite->id == $this->getMainSiteIndex()) {
            return $mainUploadUrl;
        } else {
            $mainOldUrl    = $paramsManager->getValue(DUPX_Params_Manager::PARAM_URL_OLD);
    ...
    public function getRelativeMuPlugins()
    {
        ...
        if (is_null($relativePath)) {
            $relativePath = DupProSnapLibIOU::getRelativePath(
                    DUPX_Params_Manager::getInstance()->getValue(DUPX_Params_Manager::PARAM_PATH_MUPLUGINS_NEW),
                                                                 DUPX_Params_Manager::getInstance()->getValue(DUPX_Params_Manager::PARAM_PATH_NEW)
            );
        }
        return $relativePath;
    }
    ...
    public function getPathsMapping($reset = false)
    {
        ...
        if (is_null($pathsMapping) || $reset) {
            ...
            if (
                ...
                ($subsiteId = $paramsManager->getValue(DUPX_Params_Manager::PARAM_SUBSITE_ID)) !== 1
            ) {
            ...
            foreach ($paths as $key => $path) {
                ...
                switch ($key) {
                    case 'home':
                        $newPath = $paramsManager->getValue(DUPX_Params_Manager::PARAM_PATH_NEW);
                        break;
                    case 'abs':
                        $newPath = $paramsManager->getValue(DUPX_Params_Manager::PARAM_PATH_WP_CORE_NEW);
                        break;
                    case 'wpcontent':
                        $newPath = $paramsManager->getValue(DUPX_Params_Manager::PARAM_PATH_CONTENT_NEW);
                        break;
                    case 'uploads':
                    case 'subsite_media':
                        $newPath = $paramsManager->getValue(DUPX_Params_Manager::PARAM_PATH_UPLOADS_NEW);
                        break;
                    case 'plugins':
                        $newPath = $paramsManager->getValue(DUPX_Params_Manager::PARAM_PATH_PLUGINS_NEW);
                        break;
                    case 'muplugins':
                        $newPath = $paramsManager->getValue(DUPX_Params_Manager::PARAM_PATH_MUPLUGINS_NEW);
                        break;
                    case 'themes':
                    default:
                        continue 2;
            ...
            DUPX_Log::info("--------------------------------------");
            DUPX_Log::info('PATHS MAPPING : ' . DUPX_Log::varToString($pathsMapping));
            DUPX_Log::info("--------------------------------------");
        }
        return $pathsMapping;
    }
    ...
    public function destFileFromArchiveName($pathInArchive)
    {
        ...
        foreach ($pathsMapping as $archiveMainPath => $newMainPath) {
            if (($relative = DupProSnapLibIOU::getRelativePath($pathInArchive, $archiveMainPath)) !== false) {
                return $newMainPath . '/' . $relative;
            }
        }

        // if don't find corrispondance in mapping get the path new as default (this should never happen)
        return DUPX_Params_Manager::getInstance()->getValue(DUPX_Params_Manager::PARAM_PATH_NEW) . '/' . ltrim($pathInArchive, '\\/');
    }
