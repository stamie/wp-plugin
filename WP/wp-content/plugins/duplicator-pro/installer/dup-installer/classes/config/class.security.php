...
class DUPX_Security
{
    ...
    public function check()
    {
        try {
            ...
            if (DUPX_Ctrl_ajax::isAjax($action) == true) {
                ...
                if (!isset($_REQUEST[DUPX_Params_Manager::PARAM_CTRL_ACTION])) {
                    $msg = 'Security issue'.($debug ? ' LINE: '.__LINE__.' TOKEN: '.$token.' KEY NAME: '.DUPX_Params_Manager::PARAM_CTRL_ACTION : '');
                    throw new Exception($msg);
                }
                if (!DUPX_CSRF::check($token, $_REQUEST[DUPX_Params_Manager::PARAM_CTRL_ACTION])) {
                    $msg = 'Security issue'.($debug ? ' LINE: '.__LINE__.' TOKEN: '.$token.' KEY NAME: '.DUPX_Params_Manager::PARAM_CTRL_ACTION.' KEY VALUE '.$_REQUEST[DUPX_Params_Manager::PARAM_CTRL_ACTION] : '');
                    throw new Exception($msg);
                }
                $token_tested = true;
            }
            ...
            if (($token = self::getTokenFromInput(self::ROUTER_TOKEN)) !== false) {
                if (!isset($_REQUEST[DUPX_Params_Manager::PARAM_ROUTER_ACTION])) {
                    $msg = 'Security issue'.($debug ? ' LINE: '.__LINE__.' TOKEN: '.$token.' KEY NAME: '.DUPX_Params_Manager::PARAM_ROUTER_ACTION : '');
                    throw new Exception($msg);
                }
                if (!DUPX_CSRF::check($token, $_REQUEST[DUPX_Params_Manager::PARAM_ROUTER_ACTION])) {
                    $msg = 'Security issue'.($debug ? ' LINE: '.__LINE__.' TOKEN: '.$token.' KEY NAME: '.DUPX_Params_Manager::PARAM_ROUTER_ACTION.' KEY VALUE '.$_REQUEST[DUPX_Params_Manager::PARAM_ROUTER_ACTION] : '');
                    throw new Exception($msg);
                }
                $token_tested = true;
            }
    ...
    public static function actionPasswordCheck()
    {
        ...
        $paramsManager->setValueFromInput(DUPX_Params_Manager::PARAM_SECURE_PASS, DUPX_Param_item_form::INPUT_REQUEST);

        if (($result = self::passwordArciveCheck())) {
            $paramsManager->setValue(DUPX_Params_Manager::PARAM_SECURE_OK, true);
        } else {
            $paramsManager->setValue(DUPX_Params_Manager::PARAM_SECURE_PASS, '');
            $paramsManager->setValue(DUPX_Params_Manager::PARAM_SECURE_OK, false);
        }
        $paramsManager->save();
        return $result;
    }
    ...
    public static function passwordArciveCheck()
    {
        ...
        if ($paramsManager->getValue(DUPX_Params_Manager::PARAM_SECURE_OK)) {
            return true;
        }

        $pass_hasher = new DUPX_PasswordHash(8, FALSE);
        $base64Pass  = base64_encode($paramsManager->getValue(DUPX_Params_Manager::PARAM_SECURE_PASS));
