...
MSG;
    ...
    {
        $tablePrefix = DUPX_Params_Manager::getInstance()->getValue(DUPX_Params_Manager::PARAM_DB_TABLE_PREFIX);
    ...
    public static function replaceSiteTable()
    {
        ...
        $prefix    = $paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_TABLE_PREFIX);
        ...
        $rUrlInfo = parse_url($paramsManager->getValue(DUPX_Params_Manager::PARAM_URL_NEW));
    ...
    public static function replaceBlogsTable()
    {
        ...
        DUPX_Log::resetIndent();
        $newMuUrls = $paramsManager->getValue(DUPX_Params_Manager::PARAM_MU_REPLACE);

        DUPX_Log::info("ENGINE UPDATE BLOGS TABLE\n" . DUPX_Log::varToString($newMuUrls));

        $escapedTablePrefix = mysqli_real_escape_string($dbh, DUPX_Params_Manager::getInstance()->getValue(DUPX_Params_Manager::PARAM_DB_TABLE_PREFIX));
    ...
    public static function updateTablePrefixKeys()
    {
        ...
        if (DUPX_InstallerState::isAddSiteOnMultisite()) {
            ...
            $newPrefix    = $paramsManager->getValue(DUPX_Params_Manager::PARAM_DB_TABLE_PREFIX);
        ...
        if (in_array($optionsTable, $tables)) {
            DUPX_Log::info('UPDATE PREFIX IN TABLE ' . DUPX_Log::varToString($optionsTable) . ' FROM ' . $oldPrefix . ' TO ' . $newPrefix);
        ...
        if (!DUPX_InstallerState::isAddSiteOnMultisite() && in_array($usermetaTable, $tables)) {
            DUPX_Log::info('UPDATE PREFIX IN TABLE ' . DUPX_Log::varToString($usermetaTable) . ' FROM ' . $oldPrefix . ' TO ' . $newPrefix);
        ...
        if (
            ...
            foreach ($archiveConfig->subsites as $subsiteInfo) {
                ...
                if (in_array($optionsTable, $tables)) {
                    DUPX_Log::info('UPDATE PREFIX IN TABLE ' . DUPX_Log::varToString($optionsTable) . ' FROM ' . $oldSubsitePrefix . ' TO ' . $newSubsitePrefix);
