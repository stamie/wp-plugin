...
class DUPX_CTRL
{
    ...
    public function mainController()
    {
        ...
        $ctrlAction    = $paramsManager->getValue(DUPX_Params_Manager::PARAM_CTRL_ACTION);
        $stepAction    = $paramsManager->getValue(DUPX_Params_Manager::PARAM_STEP_ACTION);

        DUPX_Log::info("\n".'---------------', DUPX_Log::LV_DETAILED);
        DUPX_Log::info('CONTROLLER ACTION: '.DUPX_Log::varToString($ctrlAction), DUPX_Log::LV_DETAILED);
        if (!empty($stepAction)) {
            DUPX_Log::info('STEP ACTION: '.DUPX_Log::varToString($stepAction));
        }
        DUPX_Log::info('---------------'."\n", DUPX_Log::LV_DETAILED);

        DUPX_Template::getInstance()->setTemplate(DUPX_Params_Manager::getInstance()->getValue(DUPX_Params_Manager::PARAM_TEMPLATE));
        ...
        switch ($ctrlAction) {
            case "ctrl-step1" :
                if ($stepAction === DUPX_CTRL::ACTION_STEP_SET_TEMPLATE) {
                    $paramsManager->setValueFromInput(DUPX_Params_Manager::PARAM_TEMPLATE);
                    DUPX_Log::info('NEW TEMPLATE:'.$paramsManager->getValue(DUPX_Params_Manager::PARAM_TEMPLATE));
                    $paramsManager->save();
                    DUPX_Template::getInstance()->setTemplate($paramsManager->getValue(DUPX_Params_Manager::PARAM_TEMPLATE));
                }
                $this->pageView = 'step1';
                break;
            case "ctrl-step2" :
    ...
    public function renderPage()
    {
        ...
        $echo                                 = false;
        $paramsManager                        = DUPX_Params_Manager::getInstance();
        $this->extraParamsPage['bodyClasses'] = 'template_'.$paramsManager->getValue(DUPX_Params_Manager::PARAM_TEMPLATE);

        if ($paramsManager->getValue(DUPX_Params_Manager::PARAM_DEBUG_PARAMS)) {
